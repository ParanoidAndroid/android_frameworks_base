 ...
 * behave, please read the <a href="{@docRoot}
 * guide/topics/fundamentals.html">Application Fundamentals</a> and <a
 * href="{@docRoot}guide/topics/fundamentals/tasks-and-back-stack.html">Tasks
 * and Back Stack</a> developer guides.
 * </p>
 * 
 * <p>
 ...
 * </div>
 * 
 * <a name="Fragments"></a> <h3>Fragments</h3>
 * 
 * <p>
 ...
 * <td align="center"><font color="#800000"><strong>Pre-
 * {@link android.os.Build.VERSION_CODES#HONEYCOMB}</strong></font></td>
 ...
 * after {@link #onPause()} and allows and application to safely wait until
...
public class Activity extends ContextThemeWrapper implements
	...
	/**
	 * true if the activity is being destroyed in order to recreate it with a
	 * new configuration
	 */
	...
	LoaderManagerImpl getLoaderManager(String who, boolean started,
			boolean create) {
	 ...
	 * {@link Intent#ACTION_ASSIST} Intent with all of the context of the
	 * current application. You can override this method to place into the
	 * bundle anything you would like to appear in the
	 * {@link Intent#EXTRA_ASSIST_CONTEXT} part of the assist Intent. The
	 ...
	 * <em>no</em> messages will be dispatched during this time (when the
	 * returned object does not have an activity to be associated with).
	 ...
	 * state is an {@link android.os.AsyncTask} you are guaranteed that its call
	 * back functions (like {@link android.os.AsyncTask#onPostExecute}) will not
	 * be called from the call here until you execute the next instance's
	...
	NonConfigurationInstances retainNonConfigurationInstances() {
		...
		if (mAllLoaderManagers != null) {
			...
			LoaderManagerImpl loaders[] = new LoaderManagerImpl[mAllLoaderManagers
					.size()];
	 ...
	 * 
	 * <p>
	 * <strong>Warning:</strong> Do not call {@link Cursor#close()} on a cursor
	 * obtained using this method, because the activity will do that for you at
	 * the appropriate time. However, if you call {@link #stopManagingCursor} on
	 * a cursor from a managed query, the system <em>will
	...
	/**
	 * @deprecated As of {@link android.os.Build.VERSION_CODES#GINGERBREAD} this
	 *             is a no-op.
	 * @hide
	 */
	@Deprecated
	...
	/**
	 * Creates a new ActionBar, locates the inflated ActionBarView, initializes
	 * the ActionBar with the view, and sets mActionBar.
	 */
	private void initActionBar() {
		...
		if (isChild() || !window.hasFeature(Window.FEATURE_ACTION_BAR)
				|| mActionBar != null) {
			return;
		}
	...
	public boolean onCreatePanelMenu(int featureId, Menu menu) {
		if (featureId == Window.FEATURE_OPTIONS_PANEL) {
			...
			show |= mFragments.dispatchCreateOptionsMenu(menu,
					getMenuInflater());
			return show;
		}
		return false;
	...
	/**
	 * Define the synthetic task stack that will be generated during Up
	 * navigation from a different task.
	 * 
	 * <p>
	 ...
	 * {@link TaskStackBuilder}. Applications may choose to override this method
	 * to construct the desired task stack in a different way.
	 * </p>
	 * 
	 * <p>
	 * This method will be invoked by the default implementation of
	 * {@link #onNavigateUp()} if {@link #shouldUpRecreateTask(Intent)} returns
	 * true when supplied with the intent returned by
	 * {@link #getParentActivityIntent()}.
	 * </p>
	 * 
	 * <p>
	 * Applications that wish to supply extra Intent parameters to the parent
	 * stack defined by the manifest should override
	 * {@link #onPrepareNavigateUpTaskStack(TaskStackBuilder)}.
	 * </p>
	 * 
	 * @param builder
	 *            An empty TaskStackBuilder - the application should add intents
	 *            representing the desired task stack
	 */
	...
	/**
	 * Prepare the synthetic task stack that will be generated during Up
	 * navigation from a different task.
	 * 
	 * <p>
	 * This method receives the {@link TaskStackBuilder} with the constructed
	 * series of Intents as generated by
	 * {@link #onCreateNavigateUpTaskStack(TaskStackBuilder)}. If any extra data
	 * should be added to these intents before launching the new task, the
	...
	public void startActivityAsUser(Intent intent, Bundle options,
		...
		Instrumentation.ActivityResult ar = mInstrumentation.execStartActivity(
				...
				-1, options, user);
		if (ar != null) {
			mMainThread.sendActivityResult(mToken, mEmbeddedID, -1,
	...
	public void startIntentSenderForResult(IntentSender intent,
		...
		startIntentSenderForResult(intent, requestCode, fillInIntent,
	...
	private void startIntentSenderForResultInner(IntentSender intent,
			int requestCode, Intent fillInIntent, int flagsMask,
			int flagsValues, Activity activity, Bundle options)
			throws IntentSender.SendIntentException {
		try {
	...
	public void startActivities(Intent[] intents, Bundle options) {
		mInstrumentation.execStartActivities(this,
				mMainThread.getApplicationThread(), mToken, this, intents,
				options);
	}
	...
	/**
	 * Same as calling
	 * {@link #startActivityFromFragment(Fragment, Intent, int, Bundle)} with no
	 * options.
	 * 
	 * @param fragment
	...
	public void startActivityFromFragment(Fragment fragment, Intent intent,
		...
		Instrumentation.ActivityResult ar = mInstrumentation.execStartActivity(
				...
				intent, requestCode, options);
		if (ar != null) {
			mMainThread.sendActivityResult(mToken, fragment.mWho, requestCode,
					ar.getResultCode(), ar.getResultData());
		}
	}
...
/**
	 ...
	 * 
	 * <p>
	 * As of {@link android.os.Build.VERSION_CODES#GINGERBREAD}, the Intent you
	 * supply here can have {@link Intent#FLAG_GRANT_READ_URI_PERMISSION
	 * Intent.FLAG_GRANT_READ_URI_PERMISSION} and/or
	 * {@link Intent#FLAG_GRANT_WRITE_URI_PERMISSION
	 * Intent.FLAG_GRANT_WRITE_URI_PERMISSION} set. This will grant the Activity
	 * receiving the result access to the specific URIs in the Intent. Access
	 * will remain until the Activity has finished (it will remain across the
	 * hosting process being killed and other temporary destruction) and will be
	 * added to any existing set of URI permissions it already holds.
	 * 
	 * @param resultCode
	 *            The result code to propagate back to the originating activity,
	 *            often RESULT_CANCELED or RESULT_OK
	...
	public void recreate() {
		if (mParent != null) {
			throw new IllegalStateException(
					"Can only be called on top-level activity");
		}
		if (Looper.myLooper() != mMainThread.getLooper()) {
			throw new IllegalStateException("Must be called from main thread");
		}
		mMainThread.requestRelaunchActivity(mToken, null, null, 0, false, null,
				false);
	}
	...
	public void finishAffinity() {
		if (mParent != null) {
			throw new IllegalStateException(
					"Can not be called from an embedded activity");
		}
		if (mResultCode != RESULT_CANCELED || mResultData != null) {
			throw new IllegalStateException(
		...
		try {
			if (ActivityManagerNative.getDefault().finishActivityAffinity(
					mToken)) {
				mFinished = true;
			}
		} catch (RemoteException e) {
	...
	public View onCreateView(View parent, String name, Context context,
		...
		int id = a.getResourceId(com.android.internal.R.styleable.Fragment_id,
				View.NO_ID);
		...
		Fragment fragment = id != View.NO_ID ? mFragments.findFragmentById(id)
				: null;
	...
	void dumpInner(String prefix, FileDescriptor fd, PrintWriter writer,
			String[] args) {
		writer.print(prefix);
		...
		if (mLoaderManager != null) {
			...
			writer.print(Integer.toHexString(System
					.identityHashCode(mLoaderManager)));
			writer.println(":");
	 ...
	 * <code>android:immersive</code> is set to <code>true</code> in the
	 * application's manifest entry for this activity, the
	 * {@link android.content.pm.ActivityInfo#flags ActivityInfo.flags} member
	 * will always have its
	 * {@link android.content.pm.ActivityInfo#FLAG_IMMERSIVE FLAG_IMMERSIVE} bit
	 * set.
	 * 
	 * @see #isImmersive()
	 * @see android.content.pm.ActivityInfo#FLAG_IMMERSIVE
	 ...
	 * @return The new action mode, or <code>null</code> if the activity does
	 *         not want to provide special handling for this action mode. (It
	...
	public boolean shouldUpRecreateTask(Intent targetIntent) {
		try {
			...
			return !ActivityManagerNative.getDefault()
					.targetTaskAffinityMatchesActivity(mToken,
							info.taskAffinity);
		} catch (RemoteException e) {
			return false;
	...
	public boolean navigateUpTo(Intent upIntent) {
		if (mParent == null) {
			...
			try {
				...
				return ActivityManagerNative.getDefault().navigateUpTo(mToken,
						upIntent, resultCode, resultData);
			} catch (RemoteException e) {
				return false;
			}
		} else {
	...
	public Intent getParentActivityIntent() {
		...
		try {
			final ActivityInfo parentInfo = getPackageManager()
					.getActivityInfo(target, 0);
			final String parentActivity = parentInfo.parentActivityName;
			final Intent parentIntent = parentActivity == null ? Intent
					.makeMainActivity(target) : new Intent()
					.setComponent(target);
			return parentIntent;
		} catch (NameNotFoundException e) {
			Log.e(TAG, "getParentActivityIntent: bad parentActivityName '"
	...
	final void attach(Context context, ActivityThread aThread,
		...
		attach(context, aThread, instr, token, 0, application, intent, info,
				title, parent, id, lastNonConfigurationInstances, config);
	}
	...
	final void attach(Context context, ActivityThread aThread,
		...
		if (intent != null && floating && !history) {
			TypedArray styleArray = context.obtainStyledAttributes(info.theme,
					com.android.internal.R.styleable.Window);
			TypedValue backgroundValue = styleArray
					.peekValue(com.android.internal.R.styleable.Window_windowBackground);

			// Apps that have no title don't need no title bar
			TypedValue outValue = new TypedValue();
			boolean result = styleArray.getValue(
					com.android.internal.R.styleable.Window_windowNoTitle,
					outValue);

			if (backgroundValue != null
					&& backgroundValue.toString().contains("light")) {
				context.getTheme()
						.applyStyle(
								com.android.internal.R.style.Theme_DeviceDefault_FloatingWindowLight,
								true);
			} else {
				context.getTheme()
						.applyStyle(
								com.android.internal.R.style.Theme_DeviceDefault_FloatingWindow,
								true);
			}
			...
			if (this instanceof LayerActivity
		...
		mWindow.setWindowManager((WindowManager) context
				.getSystemService(Context.WINDOW_SERVICE), mToken, mComponent
				.flattenToString(),
	...
	private void scaleFloatingWindow(Context context) {
		...
		if (metrics.heightPixels > metrics.widthPixels) {
			mWindow.setLayout((int) (metrics.widthPixels * 0.9f),
					(int) (metrics.heightPixels * 0.7f));
		} else {
			mWindow.setLayout((int) (metrics.widthPixels * 0.7f),
		...
		if (mWindow != null && ExtendedPropertiesUtils.isInitialized()) {
			try {
				...
				// Per-App-Color
						...
						&& ExtendedPropertiesUtils.mGlobalHook.mancol != 1
					...
					for (int i = 0; i < ExtendedPropertiesUtils.PARANOID_COLORS_COUNT; i++) {
						...
						String nextColor = (appColor == null || appColor
								.equals("")) ? colorInfo.systemColorString
								: appColor;

						// Change only if colors actually differ
						if (!nextColor.toUpperCase().equals(
								colorInfo.lastColorString.toUpperCase())) {
							ColorUtils.setColor(this, setting,
									colorInfo.systemColorString, nextColor, 1);
						}
					}
				}
			} catch (Exception e) {
	...
	final void performPause() {
		// Per-App-Extras
		if (ExtendedPropertiesUtils.isInitialized() && mParent == null
				&& mDecor != null && mDecor.getParent() != null
		...
		mResumed = false;
		if (!mCalled
				&& getApplicationInfo().targetSdkVersion >= android.os.Build.VERSION_CODES.GINGERBREAD) {
			throw new SuperNotCalledException("Activity "
					+ mComponent.toShortString()
	...
	final void performStop() {
		...
		// Floatingwindows activities should be kept volatile to prevent new
		// activities taking
		// up front in a minimized space. Every stop call, for instance when
		// pressing home,
		// will terminate the activity. If the activity is already finishing we
		// might just
		// as well let it go.
		if (!mChangingConfigurations && mWindow != null
				&& mWindow.mIsFloatingWindow && !isFinishing()) {
			finish();
		}
	}
